...
from .op.quantize import Quantize

logger = logging.getLogger('t2k.quantize')
...
def handleQuantizationTensor(TFactory, t):
    ...
    * Translate a UINT8 TFLite tensor to Quantize-Dequantize pattern.
    ...
    deqtensor.dtype = TensorType.FLOAT32
    deqtensor.setParsed()
    ...
    if t.producers:
        ...
        raise ValueError("Neither producer nor consumer available. Cannot add op")
...
def foldFP16QuantPattern(ops):
    ...
    * `FP16 input - Dequantize - FP32 tensor - Conv` -> `FP32 input - Conv`
    ...
    fp16deqs = [op for op in ops if op.type == 'DequantizeLinear' and op.inputs[0].dtype is TensorType.FLOAT16]  # noqa: E501

    count = 0
    for dep in fp16deqs:
        logger.debug("FP16 Quant Folder: Folding FP16-Quant for op %s", dep.name)
        fp16i = dep.inputs[0]
        fp16i.dtype = TensorType.FLOAT32
